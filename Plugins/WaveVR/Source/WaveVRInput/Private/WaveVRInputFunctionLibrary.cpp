...
FRotator UWaveVRInputFunctionLibrary::GetWaveVRInputDeviceAngularVelocity(EWVR_DeviceType device)
{
	EWVR_DeviceType adaptive_device = GetLeftHandedDevice(device);
	FRotator angularv = UWaveVRBlueprintFunctionLibrary::GetDeviceAngularVelocity(adaptive_device);

	return angularv;
}

FVector UWaveVRInputFunctionLibrary::GetWaveVRInputDeviceAcceleration(EWVR_DeviceType device)
{
	EWVR_DeviceType adaptive_device = GetLeftHandedDevice(device);
	FVector acceleration = UWaveVRBlueprintFunctionLibrary::GetDeviceAcceleration(adaptive_device);

	return acceleration;
}

FRotator UWaveVRInputFunctionLibrary::GetWaveVRInputDeviceAngularAcceleration(EWVR_DeviceType device)
{
	EWVR_DeviceType adaptive_device = GetLeftHandedDevice(device);
	FRotator angularacc = UWaveVRBlueprintFunctionLibrary::GetDeviceAngularAcceleration(adaptive_device);
