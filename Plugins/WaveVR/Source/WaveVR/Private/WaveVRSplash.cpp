...
bool FWaveVRSplash::ApplyTexture()
{
	...
	if (SplashTexture != nullptr && SplashTexture->IsValidLowLevel()) {
		...
		//SplashTexture->UpdateResource();    //[UE5.1] This will make texel break.
...
bool FWaveVRSplash::AllocateSplashWrapper_RenderThread()
{
	...
	ETextureCreateFlags InFlags = TexCreate_RenderTargetable;
	if(IsMobileColorsRGB())
		InFlags |= TexCreate_SRGB;

	const FRHITextureCreateDesc Desc = FRHITextureCreateDesc::Create2D(TEXT("AllocateSplashWrapper_RenderThread"), InSizeX, InSizeY, InFormat)
		.SetInitialState(ERHIAccess::CopySrc)
		.SetFlags(InFlags);
	SplashWrapperRHIRef = RHICreateTexture(Desc);
